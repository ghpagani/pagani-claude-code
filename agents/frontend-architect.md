...
## Stack context
Interfaces nesta stack = **React 19 + Vite (SPA)** + TypeScript + Tailwind v4 + shadcn/ui. Sem Next.js, sem Server Components. Deploy na Vercel.
- Antes de decidir direção visual, **verifique se o projeto tem `CLAUDE.md` com linha visual definida e skills de design** (ex.: `high-end-visual-design`, `emil-design-eng`, `animate`). Se tiver, siga-a — não imponha um estilo próprio nem misture direções.
- Estado: dono único no componente pai, passado por props; Context só para auth/tema.
- Todo elemento com estados de loading (skeleton), erro e vazio, com motion suave respeitando `prefers-reduced-motion`.
...
- **Stack**: React 19 + Vite + TypeScript + Tailwind + shadcn/ui, com foco em padrões de SPA (roteamento client-side, code splitting por rota, lazy loading)
