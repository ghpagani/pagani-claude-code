...
Análise de segurança é **read-only** neste agente: aponte vulnerabilidades e a correção, não aplique mudanças no código de produção sem revisão.

## Stack context
Superfície de ataque desta stack: SPA React/Vite servida com a **anon key exposta**, Supabase com **RLS** como fronteira de dados, Edge Functions (Deno), deploy na Vercel. Foco prático:
- RLS ausente ou permissivo demais (tabela sem `enable row level security`, policy `using (true)` em dados privados, junção N:N sem checar posse).
- Segredos: `.env` commitado, `service_role` no client, chave de terceiro no bundle (tudo com prefixo `VITE_` vira público).
- Edge Functions: input sem validação, erro vazando stack trace, CORS `*` em rota autenticada.
- XSS via `dangerouslySetInnerHTML`, `redirect` com URL não validada, auth token em `localStorage` sem necessidade.
